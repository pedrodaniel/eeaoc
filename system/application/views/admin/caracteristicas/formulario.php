...
<head>
	...
	$(document).ready(function(){
		...
		$("#btn_guardar").click(function(){
			...
			{
				jAlert("Debe ingresar el Nombre de la Caracteristica,","Error");
			}	
	...
	{	
		$("#galeria").hide();
		$("#loading").show();
		$.post("<?=site_url("admin/Caracteristicas/borrar_imagen")?>",
		{
			
			caracteristica_id: <?=$caracteristica['id']?>,
			img: "<?=$caracteristica['imagen']?>"
...
<div style='width: 100%;'>
	<div style='width: 60%; float: left; margin-right: 30px;'>		
		...
		<input name="id" id="id" type="hidden" value="<?=$caracteristica['id']?>"/>
			...
			<div id="section-detail">
				<textarea name="descripcion" id='descripcion' rows="20" style='width: 99%;'><?=(isset($caracteristica['descripcion']))?stripslashes($caracteristica['descripcion']):""; ?></textarea>
			</div>
	...
	<div style='float: left; width: 30%;'>
		 ...
		 <div id='section'>Cargar Im&aacute;gen</div>
				...
				<img src="<?=site_url("upload/caracteristica/".$caracteristica['id']."/th_".$caracteristica['imagen'])?>" />
					<a href="javascript:borrar_imagen(<?=$caracteristica['id']?>)" title="Eliminar imagen">
					Eliminar
					</a>
