...
<head>
	...
	{
		$.post("<?=site_url("admin/noticias/agregar_caracteristica")?>",
		{
	...
	{
		$.post("<?=site_url("admin/noticias/eliminar_caracteristica")?>",
		{
			relacion_id: p_id
		},
		function(data){
...
<br/>
	...
	<tbody>
		...
		<? if ($noticia_caracteristicas):?>
			<? foreach ($noticia_caracteristicas as $cp):?>
			<tr bgcolor="#ffffff">
			<td><?=$cp['id']?></td>
			<td><?=$cp['nombre']?></td>
	...
	<input name="noticia_id" type="hidden" value="<?=$noticia_id?>" id="noticia_id"/>
