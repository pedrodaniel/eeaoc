...
<head>
	...
	$(document).ready(function(){
		$("#btn_guardar").click(function(){
			...
			$("#loading").show();
			var v_dest = 0;
			if ($("#destacada").attr('checked'))
				v_dest = 1;
			$.post("<?=site_url("admin/noticias/guardar_cambios_imagen")?>",
			{
				imagen_id: <?=$imagen['id'] ?>,
				url: $("#link").val(),
				target: $("#target").val(),
	...
	function borrar()
	{
		$("#botones").hide();
		$("#loading").show();
		$.post("<?=site_url("admin/noticias/borrar_imagen")?>",
		{
			imagen_id: <?=$imagen['id'] ?>,
			noticia_id: <?=$imagen['novedad_id']?>,
			img: "<?=$imagen['img']?>"
		},function(data){
			switch(data)
			{
				...
				break;
				case "ko":
					jAlert("Error al intentar eliminar la imagen. Asegurese estar conectado.","Error");
					$("#loading").hide();
					$("#botones").show();
				break;
				case "error_permiso":
	...
	function recortar()
	{
		parent.SexyLightbox.display('<?=site_url("admin/noticias/crop/".$imagen['id'])?>?TB_iframe=true&modal=1&height=750&width=950');
	}
	...
	<div class="content">
		...
		<div style="width: 450px;">
			...
			<table cellpadding="6" border="0">
	  		...
	  		<tr>
	  			<td>Destacada </td>
	  			<td align="left"><input type="checkbox" id="destacada" name="destacada" value="1" <?=$imagen['destacada']==1?"checked":""?> style="width:5%" /></td>
	  		</tr>
	  		<tr>
	  			<td>Im&aacute;gen </td>
	  			<td><img src="<?=site_url("upload/noticia/".$imagen['novedad_id']."/th_".$imagen['img'])?>" /></td>
	  		</tr>
	  		</tbody>
	  		</table>
		</div>
