...
<head>
	...
	function buscar()
	{
		window.location="<?=site_url("admin/rubros/index")?>/"+$("#search").val();
	}
	function habilitar(p_id)
	{
		...
		if ($("#habilitado_"+p_id).attr('checked'))
			v_valor = 1;
		$.post("<?=site_url("admin/rubros/habilita")?>",
		{
			rubro_id: p_id,
			valor: v_valor
		},
...
<br/>
	...
	Rubros &nbsp;&nbsp;<span style="font-size:11px"><a href="<?=site_url("admin/rubros/formulario")?>">Nuevo</a></span>
	...
	<tbody>
		...
		<? if ($listado):?>
			...
			<td class='td-mid'><a href="<?=site_url("admin/rubros/formulario/".$rubro['id'])?>" title='Editar esta rubro'><?=$rubro['nombre']?></a></td>
			</tr>
			<? endforeach; ?>
